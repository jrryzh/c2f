...
class Fusion_UOAIS_All_VM(torch.utils.data.Dataset):
    def __init__(self, config, mode):
        super(Fusion_UOAIS_All_VM, self).__init__()
        self.config = config
        self.mode = mode
        ...
        self.uoais_all_vm_annotation_list = cvb.load("/home/zhangjinyu/uoais_new/predvms/final/all_vis_masks_test.json")
    ...
    def __len__(self):
        ...
        return len(self.uoais_all_vm_annotation_list)
    ...
    def load_item(self, index):
        ...
        img = cv2.imread(os.path.join(self.img_root_path, self.uoais_all_vm_annotation_list[index]["file_name"]), cv2.IMREAD_COLOR)
        height, width, _ = img.shape

        ann = self.uoais_all_vm_annotation_list[index]
        anno_id = ann["id"]
        img_id = ann["image_id"]
        category_id = 1
        score = ann["score"]
