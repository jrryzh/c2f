...
class Fusion_UOAIS(torch.utils.data.Dataset):
    ...
    def load_item(self, index):
        ...
        uoais_vm_annotation = self.uoais_vm_annotation_list[index]
        uoais_img_id = uoais_vm_annotation["image_id"]
        uoais_anno_id = uoais_vm_annotation["id"]
        uoais_vm_no_crop = mask_utils.decode(uoais_vm_annotation["visible_mask"])[...,np.newaxis]

        if uoais_anno_id == anno_id and uoais_img_id == img_id:
            if uoais_vm_no_crop.sum() == 0:
                print("INFO: uoais_img_id %d and uoais_vm_no_crop %d is all zeros!" % uoais_img_id, uoais_anno_id)
            else:
                vm_no_crop = uoais_vm_no_crop
        else:
            print("INFO: uoais_anno_id %d and anno_id %d not same!" % uoais_anno_id, anno_id)
            print("INFO: uoais_img_id  %d and img_id  %d not same!" % uoais_img_id, img_id)
...
class VRSP_COCOA(torch.utils.data.Dataset):
    def __init__(self, config, mode):
        super(VRSP_COCOA, self).__init__()
        self.config = config
        self.mode = mode
        self.data_info = pickle.load(open(os.path.join("/home/ubuntu/data/VRSP_COCOA_results", "retrain_{}.pkl".format(self.mode)),"rb"))
        self.label_info = np.genfromtxt(os.path.join("/home/ubuntu/data/VRSP_COCOA_results", "{}_lable.txt".format(self.mode)), dtype=np.str, encoding='utf-8')

        if self.mode=="train":
            self.img_root_path = "/home/ubuntu/data/COCOA/train2014"
        elif self.mode=="test":
            self.img_root_path = "/home/ubuntu/data/COCOA/val2014"
