...
from src.image_component import MaskedTransformer, Resnet_Encoder, Refine_Module, RGBDFusionConv, RGBDLinearFusion
...
class C2F_Seg(nn.Module):
    def __init__(self, config, mode, logger=None, save_eval_dict={}):
        ...
        self.rgbd_linearfuse_256 = RGBDLinearFusion(256).to(config.device) # 注意：假设是resnet50
        self.rgbd_linearfuse_512 = RGBDLinearFusion(512).to(config.device) # 注意：假设是resnet50
        self.rgbd_linearfuse_1024 = RGBDLinearFusion(1024).to(config.device) # 注意：假设是resnet50
        self.rgbd_linearfuse_2048 = RGBDLinearFusion(2048).to(config.device) # 注意：假设是resnet50
        ...
        param_optimizer_linearfuse_256 = self.rgbd_linearfuse_256.named_parameters()
        param_optimizer_linearfuse_512 = self.rgbd_linearfuse_512.named_parameters()
        param_optimizer_linearfuse_1024 = self.rgbd_linearfuse_1024.named_parameters()
        param_optimizer_linearfuse_2048 = self.rgbd_linearfuse_2048.named_parameters()
        ...
        optimizer_parameters = [
            ...
            {'params': [p for n, p in param_optimizer_linearfuse_256], 'weight_decay': config.weight_decay},
            {'params': [p for n, p in param_optimizer_linearfuse_512], 'weight_decay': config.weight_decay},
            {'params': [p for n, p in param_optimizer_linearfuse_1024], 'weight_decay': config.weight_decay},
            {'params': [p for n, p in param_optimizer_linearfuse_2048], 'weight_decay': config.weight_decay},
        ]
